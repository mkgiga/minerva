...
| `speech` | `speech(id, dialogue, { name?, expression? })` | Dialogue for a known actor. `name` overrides the display name. `expression` must match an entry from the actor's expressions list. |
