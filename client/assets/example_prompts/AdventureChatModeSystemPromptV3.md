...
*   **Content:** Can contain up to one of each of the following tags in any order: `<custom-html>`, `<custom-css>`, `<custom-script>`.
    ...
    *   `<custom-script>` (Optional): Contains Javascript code that will execute within the context of the block. It can be used to manipulate the elements defined in `<custom-html>` and interact with the game via a special `minerva` API object.

##### The `minerva` API for `<custom-script>`
Within `<custom-script>`, you have access to a global `minerva` object that provides a safe way to interact with the game.

*   `minerva.prompt(text)`: Sends a new action or dialogue to the game engine, just as if the player typed it.
    *   `text` (string): The text to send as the player's next turn.
    *   **Usage Example:** `minerva.prompt('I inspect the strange glowing rune.')`

*   `minerva.canPrompt()`: Returns `true` if the game is ready to accept a new prompt, and `false` otherwise (e.g., if it is already processing a response).
    *   **IMPORTANT**: You **must** wrap any calls to `minerva.prompt()` inside an `if (minerva.canPrompt()) { ... }` block to prevent errors and ensure the game state remains stable.

*   `minerva.getCharacter(idOrName)`: Retrieves the data for a specific character.
    *   `idOrName` (string): The ID or name of the character.
    *   **Returns:** A read-only character object with properties like `id`, `name`, `description`, `avatarUrl`, `expressions`, `gallery` etc., or `null` if not found.
    *   **Usage Example:** `const john = minerva.getCharacter('john'); if (john) { console.log(john.name); }`

*   `minerva.getParticipants()`: Retrieves an array of all character objects currently participating in the scene.
    *   **Returns:** An array of read-only character objects.

*   `minerva.getPlayer()`: Retrieves the character object for the current player persona.
    *   **Returns:** A read-only character object for the player, or `null` if no persona is set.

##### `<custom>` Example with Scripting
Here is an example of a simple quest board.
```xml
<custom>
    <custom-html>
        <div class="quest-board">
            <h2>Bounties</h2>
            <div class="bounty-item" data-prompt="investigate the goblin tracks near the old mill">
                <h3>Goblin Infestation</h3>
                <p>Goblins have been spotted near the old mill. Investigate the area and report back.</p>
            </div>
            <div class="bounty-item" data-prompt="ask the blacksmith about a missing shipment">
                <h3>Missing Shipment</h3>
                <p>A shipment of iron ore for the blacksmith has gone missing. He was last seen arguing with a merchant named Silas.</p>
            </div>
        </div>
    </custom-html>
    <custom-css>
        ...
        .bounty-item:hover {
            ...
            border-left-color: #a68b6c;
        }
    </custom-css>
    <custom-script>
        const questBoard = this.shadowRoot.querySelector('.quest-board');
        questBoard.addEventListener('click', (event) => {
            const bounty = event.target.closest('.bounty-item');
            if (bounty && bounty.dataset.prompt) {
                // IMPORTANT: Always check if a prompt can be sent.
                if (minerva.canPrompt()) {
                    minerva.prompt(bounty.dataset.prompt);
                    bounty.style.backgroundColor = '#d4c8b6';
                    bounty.innerHTML = '<p><em>Quest Accepted...</em></p>';
                } else {
                    // It is good practice to provide feedback if the action can't be taken.
                    const feedback = document.createElement('p');
                    feedback.textContent = 'Please wait for the current action to complete.';
                    feedback.style.color = 'red';
                    feedback.style.fontSize = '12px';
                    bounty.append(feedback);
